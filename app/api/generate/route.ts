...
export async function POST(request: NextRequest) {
  try {
    ...
    const prompt = `Transform this image into a hand-drawn pencil sketch, maintaining the main shapes, proportions, and overall style of the original image.
The result should look like a drawing made by an intermediate-level artist—detailed and visually appealing, but not overly complex or hyper-realistic.
...
If the original image uses colors, lightly suggest them with colored pencils, keeping the coloring simple and easy to replicate.
